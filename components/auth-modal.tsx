  ...
  const handleAddQuery = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        console.log("Add Query error", response.message)
        alert(response.message)
        setIsLoading(false)
      }
    } catch (error) {
      alert("Add Query error" + error)
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Papa.parse(file, {
      ...
      complete: (results: Papa.ParseResult<any>) => {
        const stringArray = results.data.flat()
        setData(stringArray);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm z-[70] flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl max-w-md w-full p-8 relative">
        ...
        {/* Header */}
        <div className="mb-8">
          <h2 className="text-2xl font-luxury font-medium text-[#2c1810] mb-2"> {mode === "query" ? "Add Query" : "Welcome to Nuvinty!"}</h2>
          <p className="text-[#6b5b4f] font-body">
            {mode === "login" ? "Log in to continue." : mode === "signup" ? "Sign up to continue." : ""}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4" autoComplete="off">
          ...
          {mode !== "query" ? (
            ...
            <div>
              ...
              <input
                ...
                className="w-full mb-3 px-4 py-3 border-2 border-[#d4c4b0] rounded-lg focus:outline-none focus:border-[#a67c52] transition-colors font-body placeholder-[#8a7960]"
              ...
              <input
                ...
                type="file"
                accept=".csv"
                key={query + Date.now()} // Add timestamp to force re-render even with same file
          ...
          {mode === "query" ? (
            <button
              type="submit"
              disabled={query.length == 0 && data.length == 0 || isLoading}
              className={`w-full py-3 px-4 rounded-lg font-luxury font-medium transition-colors ${(query.length > 0 || data.length > 0) && !isLoading
                ? "bg-[#a67c52] text-white hover:bg-[#8a6441]"
                : "bg-[#d4c4b0] text-[#8a7960]"
                }`}
            >
              {isLoading ? "Please wait..." : `Add ${data.length > 0 ? data.length : ""} Query`}
            </button>
          ) : (
            <button
              type="submit"
